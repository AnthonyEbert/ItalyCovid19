...
x_confirmed <- x %>%
  ...
  tidyr::pivot_wider(id_cols = c(`Province/State`, `Country/Region`,  "Lat", "Long"), names_from = data, values_from = totale_casi)
...
x_deaths <- x %>%
  ...
  tidyr::pivot_wider(id_cols = c(`Province/State`, `Country/Region`,  "Lat", "Long"), names_from = data, values_from = deceduti)

readr::write_excel_csv(x_deaths, "johns-hopkins-format/time_series_19-covid-Deaths_Italy.csv")

x_recovered <- x %>%
  select(-deceduti, -totale_casi) %>%
  tidyr::pivot_wider(id_cols = c(`Province/State`, `Country/Region`,  "Lat", "Long"), names_from = data, values_from = dimessi_guariti)
